...
export const createUser = async (user, navigation) => {
...
export const signin = async (user, navigation) => {
  const apiurl = `${ROOT_URL}/signin`;
  try {
    const signinrequest = await axios.post(apiurl, user);
    await AsyncStorage.setItem('token', signinrequest.data.token);
...
export const getUserRoles = async history => {
  try {
    const token =await AsyncStorage.getItem("token");
    const getRolesRequest = await axios.get(`${ROOT_URL}/getuserroles`, {
      headers: {
        Authorization: token
      }
    });
...
export const addSchool = async (school, history) => {
  ...
  try {
   ...
   await AsyncStorage.setItem("token", addSchoolRequest.data.token);
